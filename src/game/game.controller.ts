...
async function add(req: Request, res: Response) {
    try {
        const game = await em.create(Game, res.locals.sanitizedInput)
        await em.flush()
...
async function sanitizeInput(req: Request, res: Response, next: NextFunction) {
    const incoming = await validateGame(req.body)
    if (!incoming.success)
        return res.status(400).json({message: incoming.issues[0].message})
    const newGame = incoming.output
 ...
 /*   res.locals.sanitizedInput = {
    ...
    const sanitizedInput = res.locals.newGame
