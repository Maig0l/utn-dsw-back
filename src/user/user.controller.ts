...
import {
  ...
  AuthError,
  getUserTokenDataFromAuthHeader,
...
async function update(req: Request, res: Response) {
  ...
  if (image?.profile_img?.[0]) {
    input.profile_img = getImageUrl(image.profile_img[0].filename);
  }
...
async function uploadImg(req: Request, res: Response) {
  ...
  user.profile_img = getImageUrl(profile_img);
  await orm.em.flush();
...
async function changeProfilePicture(
  ...
  if (user.profile_img && user.profile_img.trim() !== "") {
    ...
    const oldFilename = user.profile_img.replace("/uploads/", "");
    const oldFile = getImagePath(oldFilename);
    if (existsSync(oldFile)) await fs.unlink(oldFile);
  }

  user.profile_img = getImageUrl(req.file.filename);
  await em.flush();
...
async function setAdminStatus(req: Request, res: Response) {
  if (!req.query.admin || !req.params.nick)
    return res
      .status(400)
      .json({ message: "Target nick and desired admin status needed." });

  const adminVal = req.query.admin as string;
  if (!adminVal.match(/^(true|false)$/i))
    return res
      .status(400)
      .json({ message: "`admin` param must be either 'true' or 'false'." });
