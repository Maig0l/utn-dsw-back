 ...
 */
export function getUserTokenDataFromAuthHeader(authHeader: string | undefined): UserAuthObject {
    if (! authHeader)
        throw new AuthError("Must log in")
    ...
    // additional checks
    if (tokenData.iss != "Wellplayed.gg" ||
        tokenData.sub != "UserDataToken"
    ) {
        throw new AuthError("Invalid token")
    }
 ...
 */
export function getUserReferenceFromAuthHeader(authHeader: string | undefined): User {
